...
async function runPlanStage(config, promptsDir, runDir, topic) {
...
async function runReviewStage(config, promptsDir, runDir, stage) {
  ...
  for (let round = startRound; round <= rounds; round++) {
    ...
    const results = await Promise.allSettled(tasks);
    let ok = 0, fail = 0;
    for (const r of results) { if (r.value?.status === "ok") ok++; else fail++; }
...
async function runStage(config, promptsDir, runDir, stageId) {
  ...
  switch (stageId) {
    ...
    case "review":
      await runReviewStage(config, promptsDir, runDir, stage);
      break;
    default:
...
async function continueWorkflow(config, promptsDir, runDir) {
  ...
  switch (meta.stage) {
    ...
    case "final_plan_done":
      await runReviewStage(config, promptsDir, runDir, config.workflow.stages.find(s => s.id === "review") || { agents: ["claude", "codex"], rounds: 2 });
      meta.stage = "review_round_1_done";
